...
local mode = 1 --As mentioned before in discord, there is only one mode here. If all modes are ever completed, everything will be merged into a single file. The other modes that may exist here could be outdated or not match the description.
...
local LeftLeg = _LeftLeg_Pivot.BASE_LeftLeg
local RightLeg = _RightLeg_Pivot.BASE_RightLeg
...
if mode == 1 --[[or mode == 2]] then
    ...
    local parts = {
        ...
        },
        {
            ...
            parents = { root, _LeftLeg_Pivot },
            custom = LeftLeg
        },
        {
            vanilla = { V_R_LEG },
            vanillaPivot = vec(2, 12, 0),
            parents = { root, _RightLeg_Pivot },
            custom = RightLeg
        },
    }
